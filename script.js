...
window.ImageMap = function (map) {
            var n,
                areas = map.getElementsByTagName('area'),
                len = areas.length,
                coords = [],
                previousWidth = $('img[usemap=#' + map.id + ']')[0].naturalWidth;
...
$(document).ready(function($){
  var overtoneSpiralMap = new ImageMap(document.getElementById('overtone-spiral'));
  
  overtoneSpiralMap.resize();
  
  $('map#overtone-spiral area').on('click', function(){
    var baseFrequency = $('#base').val(),
        idx           = $(this).index() + 1;
    console.log(baseFrequency);
    tones.play( idx * baseFrequency );
  });
